...
export class EventVenueController {
    ...
    public getAllEventVenues = async (
        ...
        try {
            const eventVenues =
                await this.eventVenueService.getAllEventVenues();
    ...
    public getEventVenueById = async (
        ...
        try {
            ...
            const event =
                await this.eventVenueService.getEventVenueById(eventVenueId);
            if (event == null) {
                return res
                    .status(404)
                    .json({ error: "Event venue does not exists" });
            }
    ...
    public updateEventVenueById = async (
        ...
        try {
            ...
            if (event == null) {
                return res
                    .status(404)
                    .json({ error: "Event venue not exists" });
            }

            const eventVenueUpdate = req.body;
            const updatedEvent =
                await this.eventVenueService.updateEventVenueById(
                    eventVenueId,
                    eventVenueUpdate,
                );
